...
export function buildClicker(
  ...
  const filledCustomOutline = (): Section => {
    const rings = params.customOutline ?? [];
    const scale = params.capWidthMm;
    const validRings = rings
      .filter((r) => r.length >= 3 && getRingArea(r) > 0.001)
      .map((r) => r.map(([x, y]) => [x * scale, y * scale] as [number, number]));
    if (validRings.length === 0) {
      return track(CrossSection.square([params.capWidthMm, params.capWidthMm], true));
    }
    let cs = track(new CrossSection(validRings, 'NonZero'));
    cs = removeHoles(cs);
    return simp(cs, 0.03);
  };
  ...
  if (params.baseShape === 'outline') {
  ...
  } else if (params.baseShape === 'custom') {
    plate = track(filledCustomOutline().offset(border, 'Round', 2.0, 32));
    plate = removeHoles(plate);
    const smoothingRadius = 2.0;
    plate = simp(track(plate.offset(smoothingRadius, 'Round', 2.0, 24).offset(-smoothingRadius, 'Round', 2.0, 24)), 0.05);
  } else {
    ...
    const genShape = (rr: number): Section => {
      const cornerR = Math.max(0.1, Math.min(params.cornerRadius ?? rr * 0.22, rr - 0.01));
      switch (params.baseShape) {
        case 'square': return roundedRect(2 * rr, 2 * rr, cornerR);
...
/** Build a small tolerance gauge: a plug sized to the cap and a ring sized to the
 *  body well using the current fit tolerance. Printed flat, it lets the user verify
 *  the slip fit before committing to a full clicker. */
export function buildGauge(wasm: Wasm, params: BuildParams): ClickerPart[] {
  const { Manifold, CrossSection } = wasm;
  const trash: { delete(): void }[] = [];
  const track = <T extends { delete(): void }>(o: T): T => {
    trash.push(o);
  ...
  const capDia = Math.max(12, params.capWidthMm);
  const tol = Math.max(0.05, params.tolerance);
  const height = 2.0;
  const wall = 3.0;

  // Plug = cap diameter.
  const plugCs = track(CrossSection.circle(capDia / 2, 96));
  const plug = track(track(Manifold.extrude(plugCs, height)).translate([0, 0, 0]));

  // Ring = well diameter (cap + 2×tol) with a thick wall.
  const ringOuter = track(CrossSection.circle(capDia / 2 + tol + wall, 96));
  const ringInner = track(CrossSection.circle(capDia / 2 + tol, 96));
  const ringFootprint = track(ringOuter.subtract(ringInner));
  const ring = track(Manifold.extrude(ringFootprint, height));

  // Place plug and ring side by side on the build plate with a small gap.
  const gap = 3.0;
  const plugX = -(capDia / 2 + tol + wall + gap) / 2;
  const ringX = (capDia / 2 + tol + wall + gap) / 2;

  const plugMoved = track(plug.translate([plugX, 0, 0]));
  const ringMoved = track(ring.translate([ringX, 0, 0]));

  const parts: ClickerPart[] = [
    toGaugePart(plugMoved, 'top', [180, 180, 185], 'gauge-plug'),
    toGaugePart(ringMoved, 'base', [120, 124, 130], 'gauge-ring'),
  ];
