...
import type { ThreeMFExportOptions } from '../export/threemfExport';
...
export interface UiState {
  ...
  /** Corner radius for square base shape (mm). */
  shapeCornerRadius: number;
  /** Custom base outline uploaded from an SVG. Kept outside the store in main.ts. */
  customOutline: Ring[] | null;
...
export interface UiCallbacks {
  ...
  onCustomOutlineUpload(file: File): void;
  ...
  onExport(options: ThreeMFExportOptions): void;
  onExportGauge(): void;
  onRenderPng(): void;
  onAiPrompt(): void;
  onSaveProject(): void;
...
export function createUi(
  ...
  sidebarLeft.innerHTML = `
    ...
    <div class="section" id="baseStyleSection">
      ...
      <div class="prow-stacked" id="cornerRadiusField">
        <div class="prow-header">
          <label for="cornerRadius">Corner radius ${tip('Rounded corners for the square shape. Does not affect the image itself.')}</label>
          <input type="text" class="val" id="cornerRadiusVal" />
        </div>
        <input type="range" id="cornerRadius" min="0" max="10" step="0.2" />
      </div>
      <div class="field" id="customOutlineField" style="margin-bottom: 12px; display:none;">
        <label class="upload-cta">
          ...
          Upload SVG outline
          <input id="customOutlineUpload" type="file" accept=".svg,image/svg+xml" hidden />
        </label>
      </div>
  ...
  sidebarRight.innerHTML = `
    ...
    <div class="sidebar-sticky-footer">
      <button class="primary" id="export" style="width:100%; margin-bottom:10px">Export 3MF</button>
      <button class="secondary" id="exportGauge" style="width:100%; margin-bottom:10px">Export fit gauge</button>
      <div id="projectSettingsContainer">
        <div class="btn-row">
          ...
          <button id="shareProj" class="secondary">Share link</button>
  ...
  // --- Corner radius + custom outline ---
  const cornerRadius = $<HTMLInputElement>('cornerRadius');
  const cornerRadiusField = $('cornerRadiusField');
  const customOutlineField = $('customOutlineField');
  const customOutlineUpload = $<HTMLInputElement>('customOutlineUpload');

  function syncShapeFields(shape: BaseShapeKind) {
    const isSquare = shape === 'square';
    cornerRadiusField.style.display = isSquare ? 'flex' : 'none';
    customOutlineField.style.display = shape === 'custom' ? 'block' : 'none';
  }

  cornerRadius.addEventListener('input', () => cb.onCornerRadius(+cornerRadius.value));
  customOutlineUpload.addEventListener('change', () => {
    const f = customOutlineUpload.files?.[0];
    if (f) cb.onCustomOutlineUpload(f);
    customOutlineUpload.value = '';
  });
  ...
  bindValInput('cornerRadiusVal', cornerRadius, cb.onCornerRadius);
  ...
  exportBtn.addEventListener('click', () => {
    ...
    overlay.innerHTML = `
      <div class="wz-modal sm">
        <div class="wz-head">Export 3MF</div>
        <div class="wz-body" style="display:flex; flex-direction:column; gap:14px; min-width:280px;">
          <div class="field">
            <label for="exportOrientation">Cap orientation</label>
            <select id="exportOrientation">
              <option value="faceDown" selected>Image face-down (FDM multicolor)</option>
              <option value="faceUp">Image face-up (resin / manual paint)</option>
              <option value="asPreview">As preview (face-up)</option>
            </select>
          </div>
          <div class="prow-stacked">
            <div class="prow-header">
              <label for="exportGap">Spacing (mm)</label>
              <input type="text" class="val" id="exportGapVal" value="5" />
            </div>
            <input type="range" id="exportGap" min="0" max="30" step="1" value="5" />
          </div>
        </div>
        <div class="wz-foot">
          <button id="exportCancel">Cancel</button>
          <button class="primary" id="exportConfirm">Download</button>
        </div>
      </div>`;
    document.body.appendChild(overlay);

    let orientation: ThreeMFExportOptions['orientation'] = 'faceDown';
    const orientSel = $<HTMLSelectElement>('exportOrientation');
    orientSel.addEventListener('change', () => {
      orientation = orientSel.value as ThreeMFExportOptions['orientation'];
    });

    const gap = $<HTMLInputElement>('exportGap');
    const gapVal = $<HTMLInputElement>('exportGapVal');
    const syncGap = () => {
      gapVal.value = gap.value;
    };
    gap.addEventListener('input', syncGap);
    gapVal.addEventListener('change', () => {
      const v = Math.max(0, Math.min(30, parseFloat(gapVal.value) || 5));
      gap.value = String(v);
      gapVal.value = String(v);
    });

    overlay.querySelector('#exportCancel')!.addEventListener('click', () => overlay.remove());
    overlay.querySelector('#exportConfirm')!.addEventListener('click', () => {
      cb.onExport({ orientation, gap: parseFloat(gap.value) || 5 });
      overlay.remove();
    });
  ...
  $('exportGauge').addEventListener('click', () => cb.onExportGauge());
  ...
  $('shareProj').addEventListener('click', () => cb.onShareProject());
  ...
  function update(state: UiState) {
    ...
    cornerRadius.value = String(state.shapeCornerRadius);
    setVal('cornerRadiusVal', state.shapeCornerRadius.toFixed(1) + ' mm');
    syncShapeFields(state.baseShape);
    ...
    const gaugeBtn = $<HTMLButtonElement>('exportGauge');
    gaugeBtn.disabled = !state.hasParts || state.building;
